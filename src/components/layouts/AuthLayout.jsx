...
import Card2 from "../../assets/images/card_new111.png"
    ...
    <div className='flex'>
      <div className="w-screen h-screen md:w-[60vw] px-12 pt-8 pb-12 ">
        <span className='flex flex-row'>
          ...
          <h2 className='text-2xl font-semibold text-purple-700'>Mate</h2>
        </span>
        {children}
      </div>
      ...
      <div className='hidden md:block h-screen w-[40vw] bg-violet-100 bg-auth-bg-img bg-cover bg-no-repeat bg-center overflow-hidden p-8 relative'>
        ...
        <div className='w-48 h-56 border-[20px] rounded-[40px] border-orange-600 absolute top-[30%] -right-10' />
        <div className='w-48 h-48 rounded-[40px] bg-green-600 absolute -bottom-7 -left-5' />

        <div className='grid grid-cols-1 z-20'>
          <StatsInfoCard
            icon={<LuTrendingUpDown />}
            label="Income ,Expenses & Finance Goals "
            value="1,32,300"
            color="bg-primary"
          />
        </div>

        <img className='rounded-[28px] w-67 lg:w-[90%] absolute bottom-31 shadow-lg ' src={Card2} alt="" />
      </div>
    </div>
  )
}
